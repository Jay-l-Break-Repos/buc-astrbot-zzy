...
Endpoints (all prefixed with /api):
    POST   /notification-template/create   – create a new template
    GET    /notification-template/list     – list all templates
    GET    /notification-template/detail   – get a single template by id
    POST   /notification-template/update   – update an existing template
    POST   /notification-template/delete   – delete a template
"""
...
class NotificationTemplateRoute(Route):
    ...
    def __init__(self, context: RouteContext, db_helper: BaseDatabase) -> None:
        ...
        self.routes = {
            "/notification-template/create": ("POST", self.create_template),
            "/notification-template/list": ("GET", self.list_templates),
            "/notification-template/detail": ("GET", self.get_template),
            "/notification-template/update": ("POST", self.update_template),
            "/notification-template/delete": ("POST", self.delete_template),
        }
    ...
    async def create_template(self):
        ...
        Response:
            { "status": "ok", "data": { <template object> } }
        """
        try:
    ...
    async def get_template(self):
        """GET /api/notification-template/detail?id=<template_id>

        Query parameters:
            id (int, required) – the template's primary key
    ...
    async def update_template(self):
        """POST /api/notification-template/update
        ...
        Response:
            { "status": "ok", "data": { <updated template object> } }
        """
        try:
            data = await request.get_json()
            if not data:
            ...
            # Normalise: strip whitespace from name if provided
            if name is not None:
                name = name.strip()
                if not name:
    ...
    async def delete_template(self):
        """POST /api/notification-template/delete
