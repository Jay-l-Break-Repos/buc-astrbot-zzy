...
class SQLiteDatabase(BaseDatabase):
    ...
    def get_base_stats(self, offset_sec: int = 86400) -> Stats:
        ...
        where_clause = ""
        if offset_sec:
            where_clause = " WHERE timestamp >= " + str(int(time.time()) - offset_sec)
        ...
        for row in c.fetchall():
            platform.append(Platform(name=row[0], count=row[1], timestamp=row[2]))
    ...
    def get_conversation_by_user_id(self, user_id: str, cid: str) -> Conversation:
        ...
        if res:
            return Conversation(
                ...
                created_at=res[3],
                updated_at=res[4],
                title=res[5] if len(res) > 5 else "",
                persona_id=res[6] if len(res) > 6 else "",
            )
        return None
    ...
    def get_conversations(self, user_id: str) -> List[Conversation]:
        ...
        for row in res:
            ...
            persona_id = row[4] if row[4] else ""
    ...
    def get_atri_vision_data_by_path_or_id(
        ...
        if res:
            return ATRIVision(
                ...
                caption=res[2],
                is_meme=res[3],
                keywords=res[4].split(",") if res[4] else [],
                platform_name=res[5],
                session_id=res[6],
                sender_nickname=res[7],
                timestamp=res[8],
            )
        return None
    ...
    def get_filtered_conversations(
        ...
        try:
            ...
            if message_types and len(message_types) > 0:
                ...
                where_clauses.append("(" + " OR ".join(message_type_conditions) + ")")
