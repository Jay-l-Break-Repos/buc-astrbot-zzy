...
class TestExtractPlaceholders:
    ...
    def test_extra_whitespace_inside_braces(self):
        # {{ username }} with varying spaces should still be extracted
    ...
    def test_mixed_valid_and_invalid(self):
        # Malformed {{ 123bad }} should NOT appear in extract results
...
class TestValidatePlaceholderSyntax:
    ...
    def test_too_long_name(self):
        long_name = "a" * 65
        errors = validate_placeholder_syntax(f"{{{{ {long_name} }}}}")
        assert len(errors) == 1
        assert "过长" in errors[0]

    def test_exactly_max_length_is_valid(self):
        name = "a" * 64
        errors = validate_placeholder_syntax(f"{{{{ {name} }}}}")
        assert errors == []

    def test_multiple_errors(self):
        body = "{{ }} {{ 123 }} {{ valid }}"
        errors = validate_placeholder_syntax(body)
        assert len(errors) == 2  # empty + digit-start

    def test_single_braces_ignored(self):
        # Single { or } should not trigger errors
...
class TestRenderTemplate:
    ...
    def test_multiple_vars(self):
        ...
        rendered, warnings = render_template(
            body, {"greeting": "Hi", "name": "Bob", "count": "5"}
        )
    ...
    def test_value_coerced_to_str(self):
        rendered, _ = render_template("Count: {{ n }}", {"n": 42})  # type: ignore[arg-type]
    ...
    def test_missing_error_no_raise_when_all_provided(self):
        rendered, warnings = render_template(
            "Hi {{ name }}!", {"name": "Dave"}, missing_strategy="error"
        )
    ...
    def test_value_with_special_chars(self):
        rendered, _ = render_template(
            "Message: {{ msg }}", {"msg": "Hello <World> & 'friends'"}
        )
...
# ===========================================================================
# Integration: validation on DB create / update
# ===========================================================================
...
class TestValidationIntegration:
    ...
    def test_invalid_body_detected_before_store(self, db):
        ...
        errors = validate_placeholder_syntax(body)
        assert len(errors) == 2
        # Application code would reject this before calling db.create_notification_template
    ...
    def test_render_with_missing_var_keep(self, db):
        ...
        db.create_notification_template(name="code_tpl", body=body)
        tpl = db.get_notification_templates()[0]
        rendered, warnings = render_template(tpl.body, {"name": "Bob"})
